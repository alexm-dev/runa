...
- **Multi-threaded engine:** Spawns 7 lightweight dedicated worker threads (with `crossbeam_channel`) for I/O, preview, find, and file operations, so UI is never blocked.
...
- **Blazingly lightweight:** The `rn` binary is compact (typically 1.4-2 MB, depending on operating system and architecture).
...
> Each release includes a `Release Attestation` json file that lets you verify the binaries were generated by the automated build process.  
>
> You can verify the integrity of the release archives using the `SHA256SUMS.txt` file in the [Release](https://github.com/alexm-dev/runa/releases) page.
>
> Unix
> ```bash
